...
flowchart TD
  ...
  D -- yes --> W[Wait out the term: nothing can be deposited]
  W --> F
...
The fill runs to the cap. Nothing that earns less than the bank is bought until it gets there — a
stock pinned as the highest priority is the one exception — and
a term already running is waited out rather than abandoning the fill.
