...
[section 4](#4-the-banks). The bank gets its own pass instead: whenever it has room for a deposit,
holdings that earn less than its rate are sold into it, weakest first (see
[section 16](#16-the-city-bank-routine)).
...
flowchart TD
  ...
  F --> B2{Kept from parking<br/>before the fill?}
  B2 -- yes --> N
  B2 -- no --> A
  B -- no --> A[acquire]
...
track the parking assigned to each acquisition. After the wait, `acquire()` re-checks the price
against real cash and waits out any shortfall before buying, so a projection that lands short can
never take the balance negative.

`potSpend()` records where each purchase's money came from, drawing on the five tracked sources in
proportion to what each holds. `potCredit()` files every inflow under one of them as it arrives,
which is what the planner's **Paid With** column reads.
</details>
...
flowchart TD
  ...
  D -- yes --> P[Park spare cash in stocks<br/>that pay out before it ends]
  P --> W[Term ends: sell the parked stocks,<br/>weakest first, until the bank's room is covered]
  W --> F
  D -- no --> L[Sell holdings that earn less<br/>than the bank, weakest first]
  L --> F[Deposit up to the cap<br/>on whichever term leaves the most money]
  F --> T[Run the term, checking weekly<br/>for income that can be parked]
  T --> M[At maturity: deposit + interest released,<br/>parked stocks sold into the bank]
  M --> C{At the cap?}
  C -- no --> F
  C -- yes --> H[Hold the highest rate indefinitely]
```

The fill runs to the cap. Nothing that earns less than the bank is kept until it gets there. A
stock pinned as the highest priority is the one exception.

A term already running can't be added to, so the cash waiting for it is **parked**: spent on stocks
that complete at least one payout before the term ends, where the payouts beat the 0.1% sale fee.
The planner tries two selections, best return per dollar and biggest return overall, and keeps
whichever earns more. When the term ends the parked stocks are sold, weakest first, until the
bank's free room is covered. Any left over once the bank is full are kept as ordinary purchases,
since they are the best-returning stocks the money would buy next anyway.

The same happens during the planner's own terms. Every 7 days it checks whether the income built up
since the deposit can buy a stock that still has a payout to come before the term ends, and parks
it. At maturity those are sold into the next deposit in the same way.

Holdings already owned are treated the same way: before each deposit, anything that earns less
than the bank's rate is sold into it, weakest first, until the room is covered. Situational,
booster and no-ROI stocks are left alone, since their worth is not cash.

### Two questions, two different answers

The routine picks its term differently depending on where it is:

> **While filling**, income can only be deposited when a term matures, so a long lock leaves it
> idle, but a longer term usually pays a higher rate. The app simulates a year of filling on every
> unlocked term, depositing each maturity's balance plus the income that built up, and picks
> whichever leaves the most money at the end of it. Income waiting for the next deposit is counted
> at the best rate it could be parked at, for half the term on average. Unlike Basic Banking, which
> puts each term's budget in at the start, income here only goes in once it has been earned.
>
...
`runCityBank()`. `richestFillOption(opts, cash, income, held, cap)` simulates a year on each term
to answer the fill question, fed `cityBankTerms()` or, on the newbie path, `bankOptionsAt(day)`;
`bestCityBankTerm()` answers the full one. `parkUntilBank()`, `settleParking()` and
`sellIntoBank()` handle the parking and the sales into the bank; `idleParkRate()` is the rate the
fill comparison credits to waiting income. `PARK_CHECK_DAYS = 7`. `bankRoiFor(row)` divides out the Investments-page TCI checkbox so the 10% can't be
